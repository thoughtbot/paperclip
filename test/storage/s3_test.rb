...
class S3Test < Test::Unit::TestCase
  ...
  context "An attachment that uses S3 for storage and has a proc for styles, url, and path" do
    setup do
      rebuild_model :styles  => lambda { |attachment| attachment.instance.counter; {:thumbnail => { :geometry => "50x50#", :s3_headers => {'Cache-Control' => 'max-age=31557600'}} }},
                    :storage => :s3,
                    :bucket  => "bucket",
                    :url => Proc.new {|attachement| dummy = "dummy_string"; "/system/:attachment/#{dummy}/:style/:basename.:extension"},
                    :s3_credentials => {
                      'access_key_id' => "12345",
                      'secret_access_key' => "54321"
                    }
